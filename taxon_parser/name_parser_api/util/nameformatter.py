...
def authorString(authors, inclYear):
    ...
    """
    sb = appendAuthorship(authors, inclYear)
    if len(sb) == 0:
        return
    return sb
...
def appendAuthorshipFromParsedName(n):
    ...
    if n.basionymAuthorship.exists():
        sb += "(" + appendAuthorship(n.basionymAuthorship, True) + ") "
    
    if n.combinationAuthorship.exists():
        sb += appendAuthorship(n.combinationAuthorship, True)
